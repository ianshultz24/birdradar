...
export async function GET(request: NextRequest) {
  ...
  if (!speciesCode || !lat || !lng) {
    return Response.json({ error: 'speciesCode, lat, and lng are required' }, { status: 400 });
  }
  ...
  const url = `https://api.ebird.org/v2/data/obs/geo/recent/${encodeURIComponent(speciesCode)}?lat=${latNum}&lng=${lngNum}&dist=${distNum}&back=30&maxResults=50&fmt=json`;
